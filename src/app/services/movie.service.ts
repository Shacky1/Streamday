...
getMovieGenres() {
  ...
  getTrending(type: 'movie' | 'tv' | 'person' = 'movie', period: 'day' | 'week' = 'day') {
    return this.http.get(`${this.baseUrl}/trending/${type}/${period}?api_key=${this.apiKey}`);
  }
  ...
  getPersonDetail(id: number) {
    return this.http.get(`${this.baseUrl}/person/${id}?api_key=${this.apiKey}&append_to_response=external_ids`);
  }
