...
import { ActivatedRoute, RouterModule } from '@angular/router'; // <-- Import RouterModule here
...
  movie: any = null;
  streamUrl: string | null = null;  // store raw string URL
...
  ngOnInit(): void {
  this.type = this.route.snapshot.paramMap.get('type') || 'movie'; // default to movie
...
    // Anime & Manga are handled from a custom source, not TMDb
    case 'anime':
...
    default:
      // If unknown type, try movie first
      this.loadMovie(+id);
      break;
  }
}
...
    const yt = res.results.find((v: any) => v.site === 'YouTube' && v.type === 'Trailer');
    if (yt) {
